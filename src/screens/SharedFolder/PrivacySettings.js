...
export default function PrivacySettings({ navigation }) {
  ...
  // Toggle Logic with useCallback to keep PrivacyRow optimized
  const handleToggle = useCallback(async (key, storageKey) => {
    setPrivacy(prev => {
      const previousValue = prev[key];
      const newValue = !previousValue;

      // Async operation inside state setter is a no-go, so we handle storage separately
      AsyncStorage.setItem(storageKey, JSON.stringify(newValue)).catch(err => {
        Alert.alert("Error", "Could not save setting.");
        // Rollback state if storage fails
        setPrivacy(current => ({ ...current, [key]: previousValue }));
      });
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={styles.header}>
        <TouchableOpacity 
          onPress={() => navigation?.canGoBack() ? navigation.goBack() : console.log("No back route")} 
