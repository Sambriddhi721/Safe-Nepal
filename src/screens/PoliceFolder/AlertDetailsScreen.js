...
export default function AlertDetailsScreen({ route, navigation }) {
  ...
  // Safety check: if no alert data is passed, show error state
  if (!alert) {
    return (
    ...
    <View style={styles.mainContainer}>
      ...
      <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.detailBody}>
          ...
          <View style={styles.statusLine}>
            <View style={[styles.riskTag, { backgroundColor: `${alert.color}20`, borderColor: alert.color }]}>
              <Text style={[styles.riskTagText, { color: alert.color }]}>
                {alert.severity ? alert.severity.toUpperCase() : 'UNKNOWN'} SEVERITY
              </Text>
            </View>
          ...
          {/* Conditional Action Buttons (Only for Active Alerts) */}
          {isActive && (
...
// Reusable component for protocol rows
...
const styles = StyleSheet.create({
  ...
  mainTitle: { color: '#fff', fontSize: 26, fontWeight: '800', marginBottom: 20, letterSpacing: -0.5 },
  infoBox: { 
